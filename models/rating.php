...
class Rating {
     ...
     */
     ...
     public static function createRating(string $Note_score, int $movie_id, int $user_id){
        ...
        try {
          ...
          $sql = "INSERT INTO `note`(`Note_score`, `movie_id`, `user_id`) VALUES (:Note_score, :movie_id, :user_id)";
     ...
     public static function getRatingByMovieId(int $movie_id, int $user_id)
     {
         // le try and catch permet de gérer les erreurs, nous allons l'utiliser pour gérer les erreurs liées à la base de données
         try {
             // Création d'un objet $db selon la classe PDO
             ...
             // stockage de ma requete dans une variable
             $sql = "SELECT * FROM `note` WHERE `user_id` = :user_id AND `movie_id` = :movie_id ORDER BY `Note_id` DESC";
